...
MatMul::MatMul(const MatMulParams &p)
    : SimObject(p),
      mul(p.mul),
      acc(p.acc),
      retryEvent([this] { retryStalled(); }, "sparta_matmul_retry_event"),
      tickEvent([this] { tick(); }, "sparta_matmul_tick_event")
{
    transpose = false;
}
...
MatMul::tick()
{
    bool hasPending = !stallMulQueue.empty() || !stallAccQueue.empty();
...
MatMul::startup()
{
    std::cout << "[MatMul] startup\n";
    schedule(tickEvent, curTick() + 1);
}
void
MatMul::startMatMul(uint64_t A_ptr, uint64_t B_ptr, uint64_t C_ptr, int _M,
                    int _N, int _K, bool t)
{
    ...
    mul->setCallback(
        [&](float product, int idx) { this->onMulDone(product, idx); });
    acc->setCallback([&](float sum, int idx) { this->onAccDone(sum, idx); });
    done = false;
    i = j = k = 0;
    if (!mul->push(A[i][k], transpose ? B[j][k] : B[k][j], i * N + j)) {
        stallMulQueue.push(
            {A[i][k], transpose ? B[j][k] : B[k][j], i * N + j});
...
void
MatMul::onMulDone(float product, int idx)
{
...
void
MatMul::onAccDone(float partial, int idx)
{
    ...
    std::cout << "[MatMul] Finished matrix multiplication @ tick " << curTick()
              << "\n";
    if (finishedCallback) {
        finishedCallback();
    }
