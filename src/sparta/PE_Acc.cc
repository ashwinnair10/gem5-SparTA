...
PE_Acc::PE_Acc(const PE_AccParams &p)
    : SimObject(p),
      ...
      computeEvent([this] { finishCompute(); }, "sparta_acc_compute_event"),
      tickEvent([this] { tick(); }, "sparta_acc_tick_event"),
      current_sum(0.0f),
      current_input(0.0f),
      remaining_ops(0)
{
...
bool
PE_Acc::push(float product, int i)
{
