...
BaselineDriverParallel::BaselineDriverParallel(
    const BaselineDriverParallelParams &p)
    : SimObject(p),
...
void
BaselineDriverParallel::startup()
{
...
void
BaselineDriverParallel::startProjection()
{
    float **out;
    float **weight;
...
void
BaselineDriverParallel::onProjectionDone()
{
    projPart++;
...
void
BaselineDriverParallel::dispatchMatMul(float **A, float **B, float **C, int M,
                                       int N, int K, bool transpose)
{
...
void
BaselineDriverParallel::onProductReady(int pe, float product, int idx)
{
...
void
BaselineDriverParallel::onAccReady(int pe, float sum, int idx)
{
    partialSums[idx] += sum;
    remainingCounts[idx]--;
...
void
BaselineDriverParallel::retryStalled()
{
    bool stalled = false;
...
void
BaselineDriverParallel::onQKDone()
{
...
void
BaselineDriverParallel::runSoftmax()
{
    for (int i = 0; i < M; i++) {
        float m = -INFINITY;
        for (int j = 0; j < N; j++) {
...
void
BaselineDriverParallel::startAV()
{
    phase = PHASE_AV;
...
void
BaselineDriverParallel::onAVDone()
{
...
void
BaselineDriverParallel::regStats()
{
