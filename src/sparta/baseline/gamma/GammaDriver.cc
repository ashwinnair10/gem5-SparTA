...
namespace gem5 {
    ...
    void GammaDriver::startup()
    {
        ...
        for (int i = 0; i < numPEs; i++)
        {
            mulUnits[i]->setCallback(
                [this, i](float product, int col)
                {
                    this->onProductReady(i, product, col);
                });

            accUnits[i]->setCallback(
                [this, i](float sum, int col)
                {
                    this->onAccReady(i, sum, col);
                });
        }
    ...
    void GammaDriver::startProjection()
    {
        float **out;
        float **weight;
    ...
    void GammaDriver::onProjectionDone()
    {
        projPart++;
    ...
    void GammaDriver::dispatchMatMul(int rows)
    {
        std::cout<< "[GAMMA] Dispatching matmul for " << rows << " rows\n";
    ...
    void GammaDriver::assignRows()
    {
        for (int pe = 0; pe < numPEs; pe++)
        {
            ...
            std::cout << "[GAMMA] Assigning row " << row
            << " to PE " << pe << "\n";
    ...
    void GammaDriver::issueRow(int pe, int row)
    {
        ...
        while (pe_k[pe] < (a_end - a_start))
        {
            ...
            while (pe_j[pe] < (b_end - b_start))
            {
                ...
                if (!mulUnits[pe]->push(a, b, j))
                {
                    std::cout << RED << "[FAILED]" <<
                    RESET << " PE " << pe
                    << " stalled on mul for row " << row
                            << " k=" << k << " j=" << j << "\n";
                    stallMulQueue.push({pe, row, pe_k[pe], pe_j[pe]});
                    std::cout << "STallMUlQueue size: "
                    << stallMulQueue.size() << "\n";
    ...
    void GammaDriver::onProductReady(int pe, float product, int col)
    {
        if (!accUnits[pe]->push(product, col))
        {
            std::cout << YELLOW << "[FAILED]" <<
            RESET << " PE " << pe <<
             " stalled on acc for product " << product
                    << " col=" << col << "\n";
            stallAccQueue.push({pe, product, col});
            std::cout << "STallAccQueue size: "
            << stallAccQueue.size() << "\n";
            if (!retryEvent.scheduled())
            schedule(retryEvent, curTick() + 1);
        }
        ...
        << RESET << " PE " << pe
        << " issued acc for product " << product
                << " col=" << col << "\n";
    ...
    void GammaDriver::onAccReady(int pe, float sum, int col)
    {
        localAcc[pe][col] += sum;
    ...
    void GammaDriver::finishRow(int pe)
    {
    ...
    // {
    //     std::cout<< "[GAMMA] Retrying stalled operations\n";
...
void GammaDriver::retryStalled()
{
    ...
    void GammaDriver::checkComplete()
    {
        if (!rowQueue.empty())
            return;
        ...
        std::cout << "[GAMMA] Matmul complete for phase "
        << phase << "\n";
        std::cout << "[GAMMA] Stall Queue\n";
        std::cout << "[GAMMA] Mul Queue Size: "
        << stallMulQueue.size() << "\n";
        std::cout << "[GAMMA] Acc Queue Size: "
        << stallAccQueue.size() << "\n";
    ...
    void GammaDriver::buildCSR(float **mat, int rows, int cols, CSR &csr)
    {
        csr.values.clear();
        csr.col_idx.clear();
        csr.row_ptr.clear();
    ...
    void GammaDriver::buildCSRTranspose
    (float **mat, int rows, int cols, CSR &csr)
    {
        csr.values.clear();
        csr.col_idx.clear();
